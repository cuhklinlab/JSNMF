...
classdef File
    ...
    methods(Static)
        ...
        function chosenFile=Ask(fileSpec, ttl)
            ...
            for i=1:N
                ...
                dscs{i}=['<html>' Html.EncodeSort('name', n) n ' | ' ...
                    Html.EncodeSort('modified', d) d ' | ' ...
                    Html.EncodeSort('size', fl.bytes) ...
                    String.encodeBytes(fl.bytes) '</html>'];
            end
            app=BasicMap.Global;
            choice=Gui.Ask(Html.WrapHr(['Select a file from<br>&nbsp;'...
                '&nbsp;&nsp;' app.smallStart allFiles(end).folder app.smallEnd]),...
                dscs,[], ttl);
            if  ~isempty(choice) && choice>0
                chosenFile=allFiles(i);
            else
                chosenFile=[];
            end
        end
        
        function [rc, out]=Diff(file1, file2)
            if ispc
                cmd='FC';
            else
                cmd='diff';
