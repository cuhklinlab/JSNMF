...
%                                  The user defined function expected by
%                                  MATLAB's knnsearch function must be of 
%                                  the form
%   
...
%                               comes at the cost of a a slight loss of
...
%                           supervisor information when 'verbose'=='graphic'.  
...
%                           server. The color format is RGB numbers from 0
%                           to 255. For example, the pink-ish colored "Large
...
%                           For example, the pink-ish colored "Large
%                           macrophages" cell subset has a label of 37143.
%                           Thus it is represented in the file
%                           <home>/Documents/run_umap/examples/colorsByName.properties 
%                           with this property setting
%                               large\ macrophages=255  204  204
%                           Because the property key is lower case the 
%                           original spelling for display in the editor table
%                           is looked for in a companion file in the same folder 
%                           with the extension spell.properties. Thus spelling 
%                           for colorsByName.properties is found in
...
%                           the same lower case property key.
%                           Default is false. 
%
...
%                           runnning with no parallelism for the "epoch
...
%                           Default is '.'
%
%   'marker_size'           The size of marker to use when 'n_components'==2
%   (v2.1.2)                and 'verbose'=='graphic'.  For options, see
%                           MATLAB's documentation for plot function.
%                           Default is 2.
%
...
%                           implementation which uses SCIPY package's eigsh.
%                           We have observed lobpcg 5 times faster with
%                           over 160k rows.
...
%                           The compression is ONLY used for eigen vectors.
...
%                          that meets the requrements if supervised_metric
...
%       with our version 2.1.01's scaling of parallelism up to the
%       # of logical CPU cores assigned to MATLAB. First, WITH version
%       2.1.01 acceleration:
%   
...
if argued.contains('randomize')
    if ~args.randomize
        warning(['No randomizing is slow and ONLY '...
            'preserves south/north/south/east info']);
    end
end
