...
if ismember(24, whichOnes)
    ...
    scale=nanstd(X(:,1:7));
    try
        [~, umap]=run_umap(X, 'metric', 'seuclidean', 'Scale', scale, 'verbose', verbose);
...
if ismember(25, whichOnes)
    ...
    cov=nancov(X(:,1:7));
    try
        [~, umap]=run_umap(X, 'metric', 'mahalanobis', 'Cov', cov, 'verbose', verbose);
        disp('ERROR ... should have failed');
    catch ex
        if ~beQuiet
            disp(ex);
...
if ismember(26, whichOnes)
    ...
    try
        cov=nancov(X);
        [~, umap]=run_umap(X, 'metric', 'MINkowski', 'Cov', cov, 'verbose', verbose);
        disp('ERROR ... should have failed');
    catch ex
        if ~beQuiet
            disp(ex);
